...
from invoice_manager.config import get_ai_provider
...
EXTRACTION_KEY = "pdf_review_extraction"
...
def matching_category_index(categories, category_name: str | None) -> int:
    if category_name:
...
def detected_invoice_type(value: str | None) -> InvoiceType:
    try:
        return InvoiceType(value) if value else InvoiceType.COST
    except ValueError:
...
methods = list(AIReviewMethod)
configured_provider = get_ai_provider()
default_method = (
    AIReviewMethod(configured_provider)
    if configured_provider in {method.value for method in methods}
    else AIReviewMethod.MOCK
)
method = st.radio(
    "Metoda ekstrakcji",
    methods,
    ...
    format_func=lambda item: {
        ...
        AIReviewMethod.OPENAI: "OpenAI API",
    }[item],
    horizontal=True,
)
if st.button(
    ...
    try:
        with st.spinner("Odczytywanie dokumentu..."):
            ...
            extraction = context.ai_review_service.extract(
                analysis.text_result.text,
                method,
            )
            st.session_state[ANALYSIS_KEY] = analysis
            st.session_state[EXTRACTION_KEY] = extraction
    except Exception as error:
        show_service_exception(error)

analysis = st.session_state.get(ANALYSIS_KEY)
extraction = st.session_state.get(EXTRACTION_KEY)
if analysis is None or extraction is None:
    st.stop()

warnings = (
    analysis.warnings
    if extraction.used_method is AIReviewMethod.LOCAL
    else [*analysis.text_result.warnings, *extraction.warnings]
)
...
st.caption(
    ...
    + {
        ...
        AIReviewMethod.MOCK: "AI demo/mock",
        AIReviewMethod.OPENAI: "OpenAI API",
    }[extraction.used_method]
)
if extraction.ai_result and extraction.ai_result.confidence is not None:
    st.metric("Pewność ekstrakcji", f"{extraction.ai_result.confidence:.0%}")
    with st.expander("Pewność poszczególnych pól"):
        st.json(extraction.ai_result.field_confidence)
...
with st.form("pdf_verification_form"):
    ...
    with first_row[1]:
        category = st.selectbox(
            "Kategoria",
            categories,
            index=matching_category_index(
                categories,
                ai_result.category if ai_result else None,
            ),
            format_func=lambda item: item.name,
        )
        invoice_type = st.selectbox(
            "Typ faktury",
            list(InvoiceType),
            index=list(InvoiceType).index(
                detected_invoice_type(ai_result.invoice_type if ai_result else None)
            ),
    ...
    detected_payment_status = (
        ai_result.payment_status
        if ai_result and ai_result.payment_status in PAYMENT_STATUS_LABELS
        else "UNPAID"
    )
    payment_status = st.selectbox(
        "Status płatności",
        list(PAYMENT_STATUS_LABELS),
        index=list(PAYMENT_STATUS_LABELS).index(detected_payment_status),
